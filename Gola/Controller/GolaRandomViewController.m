...
@interface GolaRandomViewController ()  {
...
@implementation GolaRandomViewController
...
- (instancetype)init {
    ...
    if (self) {
        GolaItem *item = [GolaItem randomItem];
...
- (IBAction)restartTapped:(id)sender {
    ...
    // 직전 item과 같은 경우
    while (_itemNumber == item.itemNumber) {
        // 같지 않을 때 까지 리셋한다
        item = [item resetItem];
    }
    _itemName = item.itemName;
